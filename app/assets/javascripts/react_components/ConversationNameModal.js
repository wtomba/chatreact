window.ConversationNameModal = React.createClass({
    propTypes: {
    ...
    statics: {
        ...
        open: function(props) {
            ...
            React.render(
                <ConversationNameModal title={props.title} name={props.name} id={props.id} />,
                $anchor.get(0)
            );
            ...
            TweenMax.from(modal, 1, { top: -$(".modal").height(), height: 0, width: wrap.width() - 40, ease: Elastic.easeOut, onComplete: function () {
              $(".name-input").focus();
            } });
        },
          ...
          <div className="modal">
            ...
            <div className="large-12 columns">
            	<form onSubmit={this.handleNameChange}>
                	...
                	<input type="text" className="name-input" placeholder={this.props.name} ref="name" required />
            		<input className="button tiny success right" type="submit" value="Ändra" />
                </form>
            </div>
          </div>
        </div>
