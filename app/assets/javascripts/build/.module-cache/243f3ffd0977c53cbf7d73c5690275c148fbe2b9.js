window.AddUserModal = React.createClass({displayName: "AddUserModal",
    ...
    statics: {
        ...
        open: function(props) {
            ...
            console.log(modal.position().left);

            TweenMax.to($("#modal-bg"), 0.75, { background: "rgba(0, 0, 0, .75)" })
            TweenMax.set(modal, { left: wrap.offset().left + padding, width: wrap.outerWidth() - padding*2, height: "auto", top: "15%" });
            TweenMax.from(modal, 1, { left: wrap.offset().left + padding, width: wrap.outerWidth() - padding*2, top: -$(".modal").height(), height: 0, ease: Elastic.easeOut });
        },
