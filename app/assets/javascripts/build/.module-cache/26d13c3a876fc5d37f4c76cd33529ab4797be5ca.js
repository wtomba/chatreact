    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.access_token) {
          localStorage.setItem("user", JSON.stringify({token: data.access_token, username: data.username, id: data.id}));
      ...
      React.createElement("form", {className: "loginForm small-12 medium-10 large-8 medium-centered large-centred columns", onSubmit: this.handleSubmit}, 
        ...
        React.createElement("div", {className: "row"}, 
          React.createElement("div", {className: "small-12 large-12 columns"}, 
            React.createElement("button", {className: "button tiny secondary left", onClick: this.handleRegistration, value: "Registrera"}), 
            React.createElement("button", {className: "button tiny success right", type: "submit", value: "Logga in"})
          )
        )
      )
    );
